...
def feature_engineering_exp4():
    codificaciones = pd.read_excel(r"/home/cnvdba/cce/src_dev/Data/Codificaciones.xlsx", sheet_name="codificaciones")
    codificaciones.Code = codificaciones.Code.str.strip()
    rename_columns ={'creation_date':"creationDate",
    ...
    dict_column_list = {"debtorParticipantCode":"participants",
    ...
    if PREDICTION_PIPELINE:
        ...
        update_process_status_to_database(run_id_max ,1,"run_process_start_datetime")

    
    # output_directory_path = '/home/cnvdba/cce/src_dev/Features/feature_engineering'
    ...
    AV_consolidado_original_values_set_stage = AV_consolidado.copy() #DO NOT DELETE THIS LINE
    ...
    vars_to_discard = ['pk', 'reasonCode','run_id','trace','instruction_id','message_id'] # 1
    vars_to_feature_engineer = ['creationDate', 'creationTime'] # 2
    vars_to_ohe = ['debtorTypeOfPerson', 'debtorParticipant', 'creditorParticipant', 'transactionType',
                    'currency', 'channel', 'responseCode'] # 3
    ...
    output_file_path = os.path.join(output_directory_path, "cce_ipf_message_feature_engineering.pickle")
    AV_consolidado_completed.to_pickle(output_file_path)

    # Export AV_consolidado_original
    output_file_path = os.path.join(output_directory_path, "cce_ipf_message_original.pickle")
    AV_consolidado_original.to_pickle(output_file_path)

    # Export AV_consolidado_original_values_set
    output_file_path = os.path.join(output_directory_path, "cce_ipf_message_original_values_set.pickle")
