...
model_path = os.path.join(current_directory, "mvp_1_model.pickle")
...
def data_preprocessing(av4_df: pd.DataFrame) -> pd.DataFrame:
    ...
    av4_df["hourSin"] = av4_df["creationTime"].apply(
        lambda x: np.sin(2 * np.pi * x.hour / 24.0)
    )
    av4_df["hourCos"] = av4_df["creationTime"].apply(
        lambda x: np.cos(2 * np.pi * x.hour / 24.0)
    )
    av4_df["creationDate"] = pd.to_datetime(av4_df["creationDate"])

    av4_df["dayOfYearSin"] = av4_df["creationDate"].apply(
        lambda x: np.sin(2 * np.pi * x.timetuple().tm_yday / 365.0)
    )
    av4_df["dayOfYearCos"] = av4_df["creationDate"].apply(
        lambda x: np.cos(2 * np.pi * x.timetuple().tm_yday / 365.0)
    )
    av4_df["dayOfMonthSin"] = av4_df["creationDate"].apply(
        lambda x: np.sin(2 * np.pi * x.day / 31.0)
    )
    av4_df["dayOfMonthCos"] = av4_df["creationDate"].apply(
        lambda x: np.cos(2 * np.pi * x.day / 31.0)
    )
    av4_df["dayOfWeekSin"] = av4_df["creationDate"].apply(
        lambda x: np.sin(2 * np.pi * x.weekday() / 7.0)
    )
    av4_df["dayOfWeekCos"] = av4_df["creationDate"].apply(
        lambda x: np.cos(2 * np.pi * x.weekday() / 7.0)
    )
    av4_df["monthSin"] = av4_df["creationDate"].apply(
        lambda x: np.sin(2 * np.pi * x.month / 12.0)
    )
    av4_df["monthCos"] = av4_df["creationDate"].apply(
    ...
    # vars_to_ohe
    av4_df["currency_soles"] = av4_df["currency"].apply(
        lambda x: 1 if x == "Soles" else 0
    )
    av4_df["responseCode_rejected"] = av4_df["responseCode"].apply(
        lambda x: 1 if x == "Rejected" else 0
    )
    ...
    av4_df[
        f'debtorParticipant_{quitar_tildes_y_espacios(av4_df["debtorParticipant"].unique()[0])}'
    ] = 1
    av4_df[
        f'creditorParticipant_{quitar_tildes_y_espacios(av4_df["creditorParticipant"].unique()[0])}'
    ] = 1
...
def get_ratios_sql(debtor_id, av4_df):
    history = get_history_sql_open(debtor_id)
...
async def fraud_prediction_sql(request: Request):
    ...
    ratios = get_ratios_sql(data["debtorId"], av4_df)
    
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"El tiempo de ejecución del sql es: {execution_time} segundos")
...
async def test_redis(request: Request):
    start_time = time.time()
    body = await request.json()
    list_transactions = get_history(body["debtor_id"])
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"Tiempo redis consulta: {str(execution_time)}")
    return {"cantidad": len(list_transactions),
            "tiempo": str(execution_time)}
    


def get_history_sql(debtor_id):
    
    connection = psycopg2.connect(**db_params)
    try:
        with connection.cursor() as cursor:
            sql_query = f"""
                SELECT * FROM coemcas.stage_ipf_message
                WHERE debtor_id = '{debtor_id}'
                AND creation_date >= CURRENT_DATE - INTERVAL '90 days';
            """
...
def get_history_sql_open(debtor_id):
...
def get_history_sql_open_2(debtor_id):
...
async def test_postgres(request: Request):
    ...
    end_time = time.time()
    execution_time = end_time - start_time
    list_transactions = get_history_sql_open_2(body["debtor_id"])
    print(f"Tiempo SQL consulta: {str(execution_time)}")
    return {"cantidad": len(list_transactions), 
            "tiempo": str(execution_time)}


@app.post("/test/timer")
async def timer_test(request: Request):
    body = await request.json()
