...
def feature_engineering_exp4():
    codificaciones = pd.read_excel(
        r"/home/cnvdba/cce/src_dev_new/Data/Codificaciones.xlsx",
        sheet_name="codificaciones",
    )
    ...
    # commented because this is the temp table, it should not affect the fraud_model_run
    # if PREDICTION_PIPELINE:
    #     update_process_status_to_database(max_run_id, 1, "run_process_start_datetime")

    # output_directory_path = '/home/cnvdba/cce/src_dev_new/Features/feature_engineering'
    ...
    for participant in participant_list:
        for bl in black_list:
            ...
            random_list = (
                generate_random_black_lists(AV_consolidado["debtor_id"], 50, i)
                if bl == "debtor"
                else generate_random_black_lists(AV_consolidado["creditor_id"], 50, i)
            )
            black_list[bl][participant] = random_list
            i += 1

    # Export black_list to pickle
    output_file_path = os.path.join(output_directory_path, "test_black_list.pickle")
    with open(output_file_path, "wb") as archivo:
        pickle.dump(black_list, archivo)

    # Create DUMMY personeria origen and personeria destino
    ...
    debtor_personeria_mapping = {
        debtor: np.random.choice(personeria_options) for debtor in unique_debtors
    }
    creditor_personeria_mapping = {
        creditor: np.random.choice(personeria_options) for creditor in unique_creditors
    }

    AV_consolidado["personeria_debtor"] = AV_consolidado["debtor_id"].map(
        debtor_personeria_mapping
    )
    AV_consolidado["personeria_creditor"] = AV_consolidado["creditor_id"].map(
        creditor_personeria_mapping
    )

    # Create DUMMY transaction amount

    np.random.seed(0)  # Optional: for reproducibility
    AV_consolidado["transaction_amount"] = np.random.uniform(
        100, 1000, size=len(AV_consolidado)
    )
    ...
    AV_consolidado["day_interval"] = AV_consolidado["creationTime"].apply(
        categorize_day_interval
    )
    # NEW VARIABLES
    if NEW_VARIABLES_FLAG:
        print("doing new variables")
        AV_consolidado["creation_date_temp2"] = pd.to_datetime(
    ...
    output_file_path = os.path.join(
        output_directory_path, "test_cce_ipf_message_feature_engineering.pickle"
    )
    AV_consolidado_completed.to_pickle(output_file_path)

    # Export AV_consolidado_original
    output_file_path = os.path.join(
        output_directory_path, "test_cce_ipf_message_original.pickle"
    )
    AV_consolidado_original.to_pickle(output_file_path)

    # Export AV_consolidado_original_values_set
    output_file_path = os.path.join(
        output_directory_path, "test_cce_ipf_message_original_values_set.pickle"
    )
