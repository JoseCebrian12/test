...
def predict_model(max_run_id, final_90_same):
    print("same input", final_90_same.shape)
    # Path and file inputs
    trained_models_path = "/home/cnvdba/cce/src/Models/Train"
    trained_model_90_old_file_name = "mvp_1_model_90_opt_old_vars_250924.pickle"
    trained_model_90_new_file_name = "mvp_1_model_90_opt_new_vars_250924.pickle"

    transformed_data_path = "/home/cnvdba/cce/src/Features/feature_engineering"
    transformed_data_file_path_90 = os.path.join(
        transformed_data_path, "df_output_90.pickle"
    )
    original_values_set_data_file_path = os.path.join(
        transformed_data_path, "cce_ipf_message_original_values_set.pickle"
    )

    # Load data
    model_90_old = load_model(trained_models_path, trained_model_90_old_file_name)
    model_90_new = load_model(trained_models_path, trained_model_90_new_file_name)
    ...
    if NEW_VARIABLES_FLAG:
        print(AV_90.columns.tolist())
        y_pred_90_new_vars = predict_isolation_forest(AV_90, model_90_new)
        y_pred_90_new_vars_same = pd.Series(
            data=[0] * len(final_90_same), index=final_90_same.index
        ).astype("uint8")
        print("size of same", y_pred_90_new_vars_same.shape)
        y_pred_90_new_vars = pd.concat(
            [y_pred_90_new_vars, y_pred_90_new_vars_same], axis=0
        )
        print("size of y pred", y_pred_90_new_vars)

        y_score_90_new_vars = score_isolation_forest(AV_90, model_90_new, 90, True)
        y_score_90_new_vars_same = pd.Series(
            data=[0] * len(final_90_same), index=final_90_same.index
        ).astype("float64")
        y_score_90_new_vars = pd.concat(
            [y_score_90_new_vars, y_score_90_new_vars_same], axis=0
        )
        # y_pred_90_new_vars.to_pickle("prediction_model_90_new.pickle")
        # y_score_90_new_vars.to_pickle("score_model_90_new.pickle")
        ...
        df_fraud_model_predict_90_new = original_values_set[
            ...
            pd.concat([AV_90, final_90_same], axis=0)[rate_features_list_90_new],
            how="inner",
            left_index=True,
            right_index=True,
        )
        print("ovs rat", df_fraud_model_predict_90_new.shape)
        ...
        # Load predictions to table

        print("PREDICTED 90 days new vars model")

    AV_90 = AV_90.drop(new_columns + new_ratios, axis=1)
    print(AV_90.columns.tolist())
    y_pred_90_old_vars = predict_isolation_forest(AV_90, model_90_old)
    y_pred_90_old_vars_same = pd.Series(
        data=[0] * len(final_90_same), index=final_90_same.index
    ).astype("uint8")
    print("size of same", y_pred_90_old_vars_same.shape)
    y_pred_90_old_vars = pd.concat(
        [y_pred_90_old_vars, y_pred_90_old_vars_same], axis=0
    )
    print("size of y pred", y_pred_90_old_vars)

    y_score_90_old_vars = score_isolation_forest(AV_90, model_90_old, 90, False)
    y_score_90_old_vars_same = pd.Series(
        data=[0] * len(final_90_same), index=final_90_same.index
    ).astype("float64")
    y_score_90_old_vars = pd.concat(
        [y_score_90_old_vars, y_score_90_old_vars_same], axis=0
    )
    # y_pred_90_old_vars.to_pickle("prediction_model_90_old.pickle")
    # y_score_90_old_vars.to_pickle("score_model_90_old.pickle")
    ...
    # y_score_90.to_pickle("score_model_90.pickle")
    ...
    df_fraud_model_predict_90_old = original_values_set[
        ...
        pd.concat([AV_90, final_90_same], axis=0)[rate_features_list_90_old],
        how="inner",
        left_index=True,
        right_index=True,
    )
    print("ovs rat", df_fraud_model_predict_90_old.shape)
    ...
    #     if col not in df_fraud_model_predict_90_old.columns:
    #         print(col)
    #         df_fraud_model_predict_90_old[col] = 0
    df_fraud_model_predict_90_old.rename(columns=new_col_names, inplace=True)

    # model_version
    load_predictions_to_database(df_fraud_model_predict_90_old, "MOD_90_OLD_260624_v2")
    load_predictions_to_database(df_fraud_model_predict_90_new, "MOD_90_NEW_260624_v2")
