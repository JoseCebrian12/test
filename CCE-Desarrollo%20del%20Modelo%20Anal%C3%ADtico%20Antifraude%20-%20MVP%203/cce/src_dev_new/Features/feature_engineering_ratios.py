...
def feature_engineering_ratio(max_run_id):
    # Specify the directory path where the .parquet files are located
    # directory_path = '/home/cnvdba/cce/src_dev_new/Features/feature_engineering'
    ...
    file_name_feature_engineering = "test_cce_ipf_message_feature_engineering.pickle"
    file_name_original_values_set = "test_cce_ipf_message_original_values_set.pickle"
    ...
    black_list_path = os.path.join(directory_path, "test_black_list.pickle")
    with open(black_list_path, "rb") as archivo:
        black_list = pickle.load(archivo)
    AV_fe["in_black_debtor"] = AV_fe["debtorId"].apply(
        lambda x: any(x in bl for bl in black_list["debtor"].values())
    )
    AV_fe["in_black_creditor"] = AV_fe["debtorId"].apply(
        lambda x: any(x in bl for bl in black_list["creditor"].values())
    )
    ...
    # Rows where either in_black_debtor or in_black_creditor is True
    final_90_black = final_90[
        final_90["in_black_debtor"] | final_90["in_black_creditor"]
    ]
    # Rows where either in_black_debtor or in_black_creditor is False
    final_90_clean = final_90[
        ~(final_90["in_black_debtor"] | final_90["in_black_creditor"])
    ]
    print("final_90", final_90.shape)
    print("final_90_black", final_90_black.shape)
    print("final_90_clean", final_90_clean.shape)

    # Only process rows that have clean clients (debtor and creditor)
    final_90 = final_90_clean
    ...
    print("final_90_diff", final_90_diff.shape)

    final_90 = final_90_diff
    print("SHAPE 90 days", final_90.shape)

    # Save the final_90 DataFrame to a pickle file
    final_90.to_pickle(os.path.join(directory_path, "test_df_output_90.pickle"))

    print(
        "FEATURE ENGINEERING RATIOS (90 days) STORED IN",
        os.path.join(directory_path, "test_df_output_90.pickle"),
    )

    return final_90_same, final_90_diff, final_90_black, final_90_clean
