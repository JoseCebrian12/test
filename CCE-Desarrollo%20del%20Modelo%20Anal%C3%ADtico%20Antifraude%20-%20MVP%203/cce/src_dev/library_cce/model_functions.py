...
PREDICTION_PIPELINE = False #Change to False if training model again
...
def deserialize_msg_data(df, header_cols, serialized_data_col):
    ...
    decoded_strings = [serialized_string.decode('utf-8') for serialized_string in tqdm(serialized_strings, total=len(df))]
    deserialized_objects = [json.loads(decoded_string) for decoded_string in tqdm(decoded_strings, total=len(df))]
    deserialized_series_list = [list(deserialized_object.values())[0] for deserialized_object in tqdm(deserialized_objects, total=len(df))]

    deserialized_data = pd.DataFrame(deserialized_series_list)

    # Combine the necessary series into a single DataFrame
    df_combined = pd.concat([series_cabecera, deserialized_data], axis=1)  # Resulting DataFrame
...
def Ratio(dd1,output_list,lista, days_back): #revisar la fecha
    ...
    if PREDICTION_PIPELINE:
        ...
        # training_days = total_days[total_days>= '2022-08-30'].reset_index(drop=True)
        # training_days = [total_days.max()]
        training_days = total_days[total_days>= '2024-09-15'].reset_index(drop=True)
...
def get_database_engine():
    ...
    engine = create_engine(f"postgresql+psycopg2://{_username}:{quoted_password}@{_server}:{_port}/{_database}")
    return engine
...
def load_predictions_to_database(df_fraud_model_predict, mod_version):
    ...
    print(f"Started saving results to database at {datetime.datetime.now()}: {df_fraud_model_predict.shape[0]} rows")
    df_fraud_model_predict.to_sql(tabla_destino, engine, if_exists='append', index=False)
...
def update_process_status_to_database(run_id,status,process_datetime_column_name):
    ...
    sql = """ UPDATE fraud_model_run SET run_process_status = %s,"""+process_datetime_column_name+"""= %s WHERE run_id = %s"""
